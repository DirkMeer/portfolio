...
def get_date(month_offset=0):
    # get current year, and month + the offset
    year, month = int(datetime.now().year), int(datetime.now().month) + month_offset
    # get the number of days in this month and pick a random day
    days_in_month = calendar.monthrange(year, month)[1]
    day = random.randrange(1, days_in_month+1)
...
def create_and_log_in_demo_user(request):
    ...
    demo_user = User.objects.create(username=username, email=user_email, password=password)
